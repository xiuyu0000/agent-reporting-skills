...
- **W26 / UI-011、QA-002、REL-012**（`done`）：PR #86（纯文档）的 firefox-smoke lane 出现一次 `@A19` 术语预览用例在 Escape 后 `toBeHidden` 超时的 flake（run 33731572904）。本机加载复现（负载均值 15–22 下 30 次 1 失败、40 次 1 失败）并从 trace 读出时间线：表头术语的预览按设计盖住表体首行的同名术语，第二次 `hover()` 只能靠 Playwright 的重试滚动通过，而这些 `scrollIntoView` 尊重模板的 `html{scroll-behavior:smooth}`，Escape 落在滚动尾部；内容随后从静止指针下滑过，浏览器合成同一锚点的 `mouseout`/`mouseover`（指针命中从锚点切到其 label span），预览被重新打开。判定为产品侧竞态：Escape 应是 WCAG 2.1 SC 1.4.13 意义上的驳回。UI-011 让被 Escape 驳回的术语在指针离开前忽略合成 `mouseover`（三引擎确定性复现修复前均重开、修复后均保持关闭；变异验证通过），QA-002 让用例以 CSSOM 关掉平滑滚动并在悬停下一术语前先 Escape，不加超时不加 retry（修复后负载均值 52–73 下 firefox 40/40）。细节见 [local-development.md](local-development.md) §5.2。字节门 391557/393216（余 1659）。候选按 REL-012 重切（ZIP 1,024,557 字节，SHA-256 `38a1e4585a5378f62c5c57f27bdb171e4e45d7c367616ae010c83bdf5d97f203`；manifest SHA-256 `5cc4f04e1a6b4753d8d1fa3ff2d949732d1fa8634dcdddbfd65225ebe8802aa1`）；tag/Release 与本机安装副本同步仍需用户授权。§5.1 的 termRef 跳转竞态仍未修复；把平滑滚动收进 `prefers-reduced-motion` 属视觉契约变更，仍待用户批准。
