...
> - Design 基线：`docs/design.md` · SHA-256 `9c88db0f53ccaafc827f821ba3c2cde357f7918f989bc9464ca9674bb4ad1a06`（2026-09-03 W25 补录 DES-022 版本线与 generatorVersion 后重算；2026-09-03 W24 值为 `13da1a4fd36d54bf64e77532970b4db15c06f4e15c67b3b6e089f6651303ba9f`；2026-09-02 W23 值为 `25952e6eb7c885ccb1c22955d53514d648613f95b647c4cc7be188501763789d`）
...
> - 目标发布：v0.2.1（候选线于 2026-09-03 由 v0.2.0 切换；`codex/v0.2.0` 冻结在 `3ff6509`，从未打 tag 或发布）
...
| UI-009 | flow 布局与标签放置消除确定性重叠 | REL-009 | ui | `done` | w24 Agent | `src/workbench/flow-layout.ts`（新增）、`flow-renderer.ts`、`shell.ts`、flow-layout/renderer 测试 | A19；用户 2026-09-02 反馈 2 |
| UI-010 | 术语定义预览脱离一切滚动裁剪容器 | REL-009 | ui | `done` | w24 Agent | `src/workbench/term-tip.ts`（新增）、`interactions.ts`、`shell.ts`、workbench-render 测试 | A19/A20；用户 2026-09-02 反馈 3 |
...
- **状态 / owner_role / owner / last_updated**：`done` / UI engineer / w24 Agent / 2026-09-02
- **Outcome**：flow 图不再出现确定性的标签重叠。节点按边集分层而不是按数组下标摆放，跨层边与回边绕列侧走，边标签在自身路径法向上做避让搜索，`viewBox` 由成品几何测得。
- **Depends on / unlocks**：REL-009 / CTR-003、REL-010。
- **Parallel / conflicts**：与 UI-010 共写 `shell.ts`，串行合并；不与 CTR-003 并行（同写 flow-renderer）。
...
- **Refs**：Design DES-021、§7.3、§11.3；Spec §7.2、§13.5；[viz-001-approval-visual-clarity-2026-09-02.md](viz-001-approval-visual-clarity-2026-09-02.md) §6 阶段 1。
...
- **Depends on / unlocks**：REL-009 / REL-010。
- **Parallel / conflicts**：与 UI-009 共写 `shell.ts`，串行合并。
...
- **Refs**：Design DES-021、§11.3；Spec §7.2 第 159 行、§13.5；[viz-001-approval-visual-clarity-2026-09-02.md](viz-001-approval-visual-clarity-2026-09-02.md) §2.2、§6 阶段 1。
...
- **状态 / owner_role / owner / last_updated**：`done` / protocol engineer / w24 Agent / 2026-09-02
- **Outcome**：审批面多出一种可视形态（强弱阶梯／光谱／占比），流程图节点与边可以声明语义类型。
- **Depends on / unlocks**：UI-009 / REL-010。
- **Parallel / conflicts**：独占公共 Schema 与生成物；不与 UI-009 并行。
...
- **Refs**：Design DES-021、§7.3、§13.2「新增节点必须同时增加 Schema、renderer、a11y 文本、正反夹具和浏览器测试」；[viz-001-approval-visual-clarity-2026-09-02.md](viz-001-approval-visual-clarity-2026-09-02.md) §3.5、§6 阶段 3。
...
- **状态 / owner_role / owner / last_updated**：`done` / surface engineer / w24 Agent / 2026-09-02
...
- **状态 / owner_role / owner / last_updated**：`done` / release engineer / w24 Agent / 2026-09-03
- **Outcome**：候选按 W24 的 workbench、协议与 Skill 变更重建，源码、生成分发面与 ZIP/manifest 再次一致。
- **Write scope**：`dist/**`、生成的 Skill 分发面、[claude-code-handoff.md](claude-code-handoff.md) §1 与 §6.2、`tests/unit/claude-handoff.test.ts`、本文摘要与证据。
...
- **Outcome**：候选线由 v0.2.0 切换到 v0.2.1。`codex/v0.2.0` 冻结在 `3ff6509`（含 W23 PR #83 与 W24 PR #84）作为从未打 tag、从未发布的 v0.2.0 候选谱系，不再合入任何提交；用户于 2026-09-03 从 `main` 开出 `codex/v0.2.1` 作为新的集成分支与全部 PR 的 base，`main` 继续以 `-s ours` 同步合并与之保持树一致。版本常量、CI、候选产物与测试字面量整体移到 0.2.1，候选按同一源码重建。
...
- **Blocker / unblock**：v0.2.1 tag 与 GitHub Release 未创建，需用户单独授权；本机 ~/.claude/skills 与 ~/.agents/skills 的安装副本仍是 2026-09-02 的 977115 字节 0.2.0 构建，未同步为本候选，需用户授权后按 DOC-003 的平台更新指南执行。
...
2026-09-02 的 W24 记录（审批面表达清晰度；候选重切）：用户在真实使用审批 HTML 后提出三条问题——(1) 内容精炼但不直白，逻辑/流程/复杂概念的可视化不足，理解门槛偏高；(2) 流程图的边标签与箭头线重叠，且可视化形式过于单一；(3) 术语悬停解释在表格靠上位置被截断。变更请求与影响判定落在 [viz-001-approval-visual-clarity-2026-09-02.md](viz-001-approval-visual-clarity-2026-09-02.md)，用户于同日批准全部三项裁决。
...
**字节预算是本轮的实际约束**：起始余量 23583 字节（369633/393216），落地 391376/393216，余 1840 字节。2026-09-03 重排到 W23（多文本节点载荷读取）之上后 shell 为 391339/393216 字节，余 1877 字节。分配为 flow 布局与提示 9772、flow `kind` 2068、`scale` 9007（其中 6775 仅来自成为 `ContentNode` 联合成员这一事实）、预览滚动跟随 137、自环标签外移 13，复审阶段的正确性修复（精确线段相交、cubic 采样、水平离屏判定、焦点保持、提示语言标注、预览高度上限）再用 1199，另有 372+81 字节由边几何与采样器去重、以及删除三个未引用的 i18n 键回收。原计划的第二个新节点 `matrix`（选项对比矩阵）因此**未实施**——任何新联合成员的地板价约 6.8 KB，实测装不下。这是按红线 L3 的取舍，不是遗漏。
