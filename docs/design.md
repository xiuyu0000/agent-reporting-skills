...
侧栏随手记编辑器必须显示写入目标块（`记到块 B00x · 标题`，2026-08-19 补充）：随手记在协议层始终绑定具体块，编辑器不可见的绑定曾被用户判定为交互缺陷。术语（termRef）是单一锚点（2026-08-20 W17 按用户反馈简化）：悬停/聚焦预览定义、激活即跳转文件内术语表附录；不再有块内展开按钮。预览仅作补充，判断所需定义的文件内载体是附录本身（spec §7.2 约束继续满足）。预览的载体自 2026-09-02（W24 / DES-021）起是挂在 `<body>` 上的单例固定定位元素，在支持时再叠加 `popover` 进入 top layer：此前的 `::after` 伪元素位于表格、代码块与侧栏的滚动容器内，而 CSS 在一轴为非 `visible` 时把另一轴计算为 `auto`，因此靠上位置的定义会被容器整块裁掉。单例随锚点滚动重定位、锚点离开视口才隐藏，并可悬停、可 `Esc` 关闭（WCAG 2.1 SC 1.4.13）；`Esc` 只关闭预览，不吞掉 §11.4 的块级键盘契约。`Esc` 是一次驳回而不是瞬时关闭（2026-09-03 W26 补充）：浏览器会在静止指针下方的内容滚动或回流时合成 `mouseover`/`mouseout`（指针命中从锚点切到它自己的 label span 即足以触发），被 `Esc` 关闭的预览因此曾在按键之后自行回来；现在同一锚点的 `mouseover` 在指针真正离开该术语（`mouseout` 的 `relatedTarget` 不在锚点之内或为 `null`）之前一律忽略，聚焦该术语或悬停其他术语则照常预览。CSS anchor positioning 不作为机制，定位由脚本计算——它在三引擎矩阵中只有 Chromium 支持。

flow 的布局同样在 2026-09-02 由 DES-021 固定：节点位置按边集分层（回边先经深度优先分类后排除，再对无环子图做最长路径分层），层内按前驱重心排序；跨层边与回边绕列侧走，不穿过节点列；平行边与互逆边按索引扇开；边标签在自身路径的法向上做穷尽候选搜索，避开节点框、其他标签与其他边路径，落位失败时取重叠最小者并标记 `crowded`；标签底板取 `--page` 实心色而非描边光晕。全部几何为纯算术，不读取任何浏览器测量 API。`viewBox` 由成品几何测量得出而不是固定 720，窄视口下的缩放因此不再把标签压到不可读。过长的边标签在图上省略，作者原串完整保留在文字清单与该标签的 `<title>` 中。
