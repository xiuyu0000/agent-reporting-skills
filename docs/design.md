...
| DES-020 | 审批面表达清晰度 | flow 采用按边集分层的确定性布局与带避让的标签放置；术语预览改为 body 级单例，脱离一切滚动容器；`ContentNode` 增加 `scale`，`FlowGraphNode`/`FlowGraphEdge` 增加可选 `kind`；Skill 写作规范以“载体选择”而非“是否配图”作强制触发 | 不提高工作台字节门，不新增 `:root` token，不改阅读顺序，不把「每个复杂块必须配图」写成规则 | 消除确定性的标签重叠与提示截断；把 spec §7.2 已授予但未被使用的可视化能力变成可执行的写作规范 |
...
DES-020 于 2026-09-02 记录：用户在真实使用后报告三条问题——表达不够直白且可视化不足、流程图标签与箭头线确定性重叠、表格靠上位置的术语悬停定义被截断。影响审计（[viz-001-approval-visual-clarity-2026-09-02.md](viz-001-approval-visual-clarity-2026-09-02.md)）判定三者都不触及 spec 需求条款：spec §8.2/§8.3 从不枚举内容节点种类，节点分类学属本文 §7.3；§13.2 早已把 `ContentNode` 定为扩展点；边标签几何与提示定位都不在 §11.8 的规范清单内。四条红线在实施全程成立：未改任何 `:root` token 或 1280/340 布局常量、未改阅读顺序、未提高 `393216` 字节门、写作规范只强制结构不强制配图。字节预算是本轮的实际约束：起始余量 23583 字节，落地后为 1840 字节，因此原计划的第二个新节点（选项对比矩阵）未实施——单是加入 `ContentNode` 联合就要约 6.8 KB 的生成校验器代码，实测确认放不下。
...
未来不兼容变化使用新的协议主版本，例如 `review-packet/2`。`/1` 的封闭对象不得增加**必填**字段；任何改变既有文档有效性的字段新增都发布新的协议版本。这样与 `additionalProperties: false` 和未知字段失败关闭保持一致。

2026-09-02（W23 / DES-020）按用户批准收窄了上一句的口径，原文为「`/1` 的封闭对象不得增加字段；任何字段新增都发布新的协议版本」。收窄的理由是该禁令原本要防的是**旧文档在新校验器下失效**，而纯可选字段不产生这种失效：`FlowGraphNode.kind` 与 `FlowGraphEdge.kind` 都不在 `required` 里，任何既有文档不设它们就逐字节不变，摘要与冻结块批准记录随之不变。风险方向相反且必须显式承认——**版本偏斜是向前的**：升级后的 Skill 写出的文档在旧运行时（发布 ZIP 自带 digest 钉死的 Schema 副本）下会以 `SCHEMA_ADDITIONAL_PROPERTIES` 或联合判别失败**拒收**，不会误读。因为 Skill 与其运行时同一个 ZIP 一起发布，混版使用本就不在支持路径内。同理，`ContentNode` 新增联合成员（`scale`）由 §13.2 的扩展点授权，旧 `/1` 校验器遇未知 `type` 在每个分支上失败关闭。必填字段新增、既有字段收紧与语义变更仍然必须走 `/2`。
...
| `flow` | `title`、`description`、`nodes: Array<{id,label,kind?}>`、`edges: Array<{from,to,label?,kind?}>` | title/description/nodes 必填且非空；节点 `id` 是该 flow 内唯一的 `^[A-Za-z][A-Za-z0-9_-]{0,31}$` 本地标识，label 非空；from/to 必须引用同一 flow 的节点，边 label 可选且非空；节点 `kind` 取 `start`/`step`/`decision`/`end`，边 `kind` 取 `then`/`yes`/`no`/`else`，两者均可选且只影响呈现与文字清单措辞；renderer 生成受控 SVG，并从节点标签与边关系确定性生成同页可展开的文字清单 |
| `scale` | `title`、`description`、`axis: {lowLabel,highLabel}`、`items: Array<{label,position,display?,note?}>` | 用于强弱阶梯、光谱与占比；items 至少两项，`position` 是 0–100 的整数；renderer 输出真实文本加成比例条，可见 DOM 即等价文字，不另出折叠清单；`display` 由作者给出，renderer 不做数字或本地化格式化以保持字节确定 |
...
侧栏随手记编辑器必须显示写入目标块（`记到块 B00x · 标题`，2026-08-19 补充）：随手记在协议层始终绑定具体块，编辑器不可见的绑定曾被用户判定为交互缺陷。术语（termRef）是单一锚点（2026-08-20 W17 按用户反馈简化）：悬停/聚焦预览定义、激活即跳转文件内术语表附录；不再有块内展开按钮。预览仅作补充，判断所需定义的文件内载体是附录本身（spec §7.2 约束继续满足）。预览的载体自 2026-09-02（W23 / DES-020）起是挂在 `<body>` 上的单例固定定位元素，在支持时再叠加 `popover` 进入 top layer：此前的 `::after` 伪元素位于表格、代码块与侧栏的滚动容器内，而 CSS 在一轴为非 `visible` 时把另一轴计算为 `auto`，因此靠上位置的定义会被容器整块裁掉。单例随锚点滚动重定位、锚点离开视口才隐藏，并可悬停、可 `Esc` 关闭（WCAG 2.1 SC 1.4.13）；`Esc` 只关闭预览，不吞掉 §11.4 的块级键盘契约。CSS anchor positioning 不作为机制，定位由脚本计算——它在三引擎矩阵中只有 Chromium 支持。

flow 的布局同样在 2026-09-02 由 DES-020 固定：节点位置按边集分层（回边先经深度优先分类后排除，再对无环子图做最长路径分层），层内按前驱重心排序；跨层边与回边绕列侧走，不穿过节点列；平行边与互逆边按索引扇开；边标签在自身路径的法向上做穷尽候选搜索，避开节点框、其他标签与其他边路径，落位失败时取重叠最小者并标记 `crowded`；标签底板取 `--page` 实心色而非描边光晕。全部几何为纯算术，不读取任何浏览器测量 API。`viewBox` 由成品几何测量得出而不是固定 720，窄视口下的缩放因此不再把标签压到不可读。过长的边标签在图上省略，作者原串完整保留在文字清单与该标签的 `<title>` 中。
...
- 重复触发同一动作遵循 `aria-pressed` 的开关语义（2026-08-19 补充，spec §9.1 的撤销要求）：块已记录 `PASS` 时，再次点击该 chip 或按 `1` 直接撤销回待处理；`EDIT`/`TOPIC`/`HOLD` 重复触发打开预填编辑器而不是静默撤销——一次按键不得销毁已输入的审批文字——撤销是编辑器内与块状态芯片上的显式按钮。
