...
侧栏随手记编辑器必须显示写入目标块（`记到块 B00x · 标题`，2026-08-19 补充）：随手记在协议层始终绑定具体块，编辑器不可见的绑定曾被用户判定为交互缺陷。术语（termRef）是单一锚点（2026-08-20 W17 按用户反馈简化）：悬停/聚焦预览定义、激活即跳转文件内术语表附录；不再有块内展开按钮。预览仅作补充，判断所需定义的文件内载体是附录本身（spec §7.2 约束继续满足）。
