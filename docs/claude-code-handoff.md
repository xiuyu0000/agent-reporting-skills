# Claude Code handoff: `deliver-dual-audience-report` v0.2.1

This is the portable operational handoff for continuing validation after the
v0.2.1 release candidate. It is detailed enough for Claude Code to work without
...
| Item | Verified value |
|---|---|
| Candidate branch | `codex/v0.2.1` (opened by the user from `main` on 2026-09-03; integration branch and PR base) |
| Frozen v0.2.0 line | `codex/v0.2.0` at `3ff6509`; its ZIP `cd103f2f…` / manifest `189c7638…` were removed from the tree by REL-011 and survive only in history; annotated tag `v0.2.0` added 2026-09-03 at that commit |
...
| Current artifact source | W9–W11, W15, W17, W19, W23 (UI-008 multi-text-node payload reader, VAL-004 payload-budget warnings), then W24 (UI-009/UI-010/CTR-003/SKL-004 approval-clarity work), then W25 (REL-011 v0.2.1 line switch: package, `RELEASE_VERSION` and `generatorVersion` 0.2.1), then W26 (UI-011: a term preview dismissed with Escape stays dismissed until the pointer leaves the term, instead of reopening on a browser-synthesized mouseover; QA-002 made the glossary-preview browser test deterministic), cut by REL-012 on 2026-09-03 |
| Candidate ZIP | `dist/deliver-dual-audience-report-v0.2.1.zip` |
| ZIP size / SHA-256 | `1024557` bytes / `38a1e4585a5378f62c5c57f27bdb171e4e45d7c367616ae010c83bdf5d97f203` |
| Manifest SHA-256 | `5cc4f04e1a6b4753d8d1fa3ff2d949732d1fa8634dcdddbfd65225ebe8802aa1` |
| Runtime | Node `>=24 <25` |
| Protocol surface | `review-document/1` gained optional-only additions in W24 (`scale` node; `kind` on flow nodes and edges). Existing documents are byte-identical and their digests unchanged. The skew is **forward**: a document written by this candidate is **rejected**, never misread, by an older v0.2.0 runtime, whose ZIP carries its own digest-pinned Schema copy. Skill and runtime ship in one ZIP, so mixed-version use is outside the supported path. W25 changed only `generatorVersion` (now `0.2.1`, the sole supported value per DES-014's no-dual-track rule); `review-document/1` and existing documents are unchanged and re-render cleanly. Views generated by a 0.2.0 build remain self-contained offline workbenches (each embeds its own 0.2.0 shell): they still open in the browser and their review round can be finished, and `META_IDENTITY_MISMATCH` does **not** occur for them (that code fires only when a payload/meta of one generator version is loaded into a shell of another, which the CLI never produces). What changes is the CLI: this candidate refuses to operate on 0.2.0-generated views — `validate delivery` reports `CSP_INVALID` at `/approval` and `/approval/csp`, and `render --replace-generated` reports `CSP_INVALID` (`/approval`, `/approval/csp`) plus `ARTIFACT_IDENTITY_MISMATCH` (`/approval/meta`) — because the old shell's inline-script/CSP hashes and generator meta differ from what the current runtime would generate. Re-rendering the unchanged `review-document.json` with 0.2.1 reproduces the same `dar-review-digest`, `contentVersion` and `round` (only `dar-generator-version` changes 0.2.0 → 0.2.1); `review-packet/1` receipts bind to the document identity (id, title, contentVersion, round, reviewDigest), not to the Approval HTML, so a receipt exported from an old view stays valid for `validate packet` / `consume` against the unchanged document. Keep old views as an archive (留档) and never delete anything under the private output root (render is create-only: `TARGET_EXISTS`); when the CLI must validate or replace a view with the new runtime, render into a fresh empty directory (or move the old views aside) from the unchanged `review-document.json`. Verified empirically on 2026-09-03 by rendering a W23-era fixture with the installed 0.2.0 runtime and running the 0.2.1 CLI on it. |
| v0.2.1 tag / GitHub Release | Not created; requires separate user authorization. The frozen line was tagged retroactively on 2026-09-03 as `v0.2.0` at `3ff6509` (annotated tag only, no GitHub Release; not a rollback baseline) |
...
in the shipped runtime and REL-004 rebuilt the archive; on 2026-09-03 the user
opened the v0.2.1 line (`codex/v0.2.1`, cut from `main`), and REL-011 re-cut the
candidate under the new version. `952704` bytes /
...
tree-identical to the integration branch by an `-s ours` sync merge; since
2026-09-03 that branch is `codex/v0.2.1`, which is the base for PRs, while
`codex/v0.2.0` is frozen at `3ff6509` as the v0.2.0 candidate lineage (never
tagged, never released, no further merges), and a pilot still runs
...
contains exactly the installed Skill runtime and its 12 release files. For a
...
| `design.md` | `d76645b23fca5588e853e236ebff8ded1998d099185c7650ef5b65893fb4ff5b` | `4c97ab3dd4ced8f3e96c514375ef9b799fe4351facc4fb724fe3dc0e8c058b79` |
| `task.md` | `64cf1ff47646ac2b6cd01108f8ad1dfebf634fb81cfd2159582462ec9913beaa` | `3287e29184d422f9a059bce9a2cdbbce4efacf58a7d971f1eec0cf61871f46df` |
...
adding any product capability. `design.md` has changed seven times: on 2026-08-18
...
2026-09-03); and on 2026-09-03 W26 amended §11.3 with the Escape-dismissal
semantics of the term preview — a preview closed with Escape stays closed while
the pointer rests on the same term, because browsers synthesize mouseover
without pointer motion whenever content scrolls or reflows under a resting
pointer (re-cut by REL-012). No engineering decision outside DES-017, DES-019,
DES-020 and DES-021 moved in any of those revisions. `task.md` changes
...
npm run verify:dist
printf '%s  %s\n' \
  '38a1e4585a5378f62c5c57f27bdb171e4e45d7c367616ae010c83bdf5d97f203' 'dist/deliver-dual-audience-report-v0.2.1.zip' \
  '5cc4f04e1a6b4753d8d1fa3ff2d949732d1fa8634dcdddbfd65225ebe8802aa1' 'dist/deliver-dual-audience-report-v0.2.1.manifest.json' \
  | shasum -a 256 -c -
```
...
unzip -q dist/deliver-dual-audience-report-v0.2.1.zip -d '<private-runtime-root>'
...
open a PR only with current user authorization. Do not tag v0.2.1 (or
retroactively v0.2.0), create a GitHub Release, publish a ZIP, close #61/#62,
or disclose pilot results without separate authorization.
