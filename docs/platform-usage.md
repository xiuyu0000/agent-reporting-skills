...
“替换已安装版本”这一动作本身未在本轮逐平台实测，凡属推断处均已标注。本仓库
维护机上的 `~/.claude/skills/` 与 `~/.agents/skills/` 副本已于 2026-09-03 经用户授权按本节
步骤同步为 v0.2.1 候选（ZIP 摘要核对通过，12 个文件逐一与 manifest 一致，`--help` 可运行）；
该次实测只覆盖“目录替换 + 逐文件核对”，各平台会话内的生效时机仍属推断。
